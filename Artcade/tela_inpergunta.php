...
<nav class="navbar navbar-expand" style="background-color:#B37549">
    ...
    <div style="margin-top:70px">
        <div class="d-flex justify-content-center">
            <div>
                <h1 class="d-flex justify-content-center subtitulo">Inserção de pergunta</h1>
                <form method="POST" action="inpergunta.php" style="width:500px">
                    <div class="form-group">
                        <input type="text" class="form-control form-control-lg" id="pergunta" name="pergunta" placeholder="Pergunta">
                    </div>
                    <div class="form-group">
                        <input type="text" class="form-control form-control-lg" id="resposta" name="resposta" placeholder="Resposta">
                    </div>
                    <div class="form-group">
                    <select class='botao-padrao' style='width:100%' id="obra" name="obra" placeholder="Obra">
                        <?php $qobras = mysqli_query($conn,"select * from obras");
                        while($row = mysqli_fetch_assoc($qobras)){
                            $titulo = $row['nome_obra'];
                            $id = $row['id_obra'];
                            echo"<option value='$titulo'>$id - $titulo</option>";
                        } ?>
                    </select>
                    </div>
