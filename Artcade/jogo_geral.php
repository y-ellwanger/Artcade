...
$rand = "select * from obras inner join artistas on obras.fk_artista=artistas.id_artista order by RAND() LIMIT 1;";// pega obra+artista baseado no periodo
...
$qpontuacao = "select * from pontuacoes where fk_usuario=".$_SESSION['id']." and fk_periodo=3 and fase_pontuacao=1;";
...
if ($id_p==1){
    ...
    $array = [];
    $fakequerys = "select * from perguntas where id_pergunta>3 order by RAND() limit 3;";
    $resul = mysqli_query($conn,$fakequerys);

    while ($row = mysqli_fetch_assoc($resul)){
        $pergunta = $row['resposta_pergunta'];
        array_push($array, $pergunta);
    }
    array_push($array,$rp);
    shuffle($array);
...
echo  ("
                ...
                <div class='justify-content-center d-flex' style='margin-top:10px;' >
                    <button class='botao-padrao' style='margin:5px;font-size:20px;padding:5px 10px' onclick='checaRes1()'>".$array[0]."</button>
                    <button class='botao-padrao' style='margin:5px;font-size:20px;padding:5px 10px' onclick='checaRes2()'>".$array[1]."</button>
                    <button class='botao-padrao' style='margin:5px;font-size:20px;padding:5px 10px' onclick='checaRes3()'>".$array[2]."</button>
                    <button class='botao-padrao' style='margin:5px;font-size:20px;padding:5px 10px' onclick='checaRes4()'>".$array[3]."</button>
                </div>
            </div>
        </div>
    </div>
